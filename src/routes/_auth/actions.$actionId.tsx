...
export const Route = createFileRoute("/_auth/actions/$actionId")({
...
function WhyRecommendationDrawer({
  ...
  const matchedBiomarkers = triggeringBiomarkers
    .map((code) => {
      // Remove status indicators like * or # from the code
      const cleanCode = code.replace(/[*#]/g, "");
      return allBiomarkers.find(
        (b) =>
            ...
            return (
              <button
                ...
                onClick={() => {
                  onOpenChange(false);
                  if (biomarker.id) {
                    navigate({ to: `/biomarkers/${biomarker.id}` });
                  }
                }}
  ...
  return (
    <div className="flex gap-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {lines.map((line, idx) => (
          <p key={idx} className="caption-md text-muted-foreground">
            {line}
          </p>
        ))}
      </div>
...
function ReferenceItem({
  ...
  item: { title: string; tags?: string[]; url?: string; createdAt?: string };
}) {
  ...
  const content = (
    <div className="bg-card rounded-2xl p-4 w-full border border-border">
      ...
      <div className="flex items-center justify-between">
        {year && (
          <span className="caption-md text-muted-foreground">{year}</span>
        )}
        {journal && (
          <span className="caption-md text-primary font-medium px-3 py-1 bg-primary/10 rounded-full">
            {journal}
          </span>
        )}
      </div>
  ...
  return (
    <div>
      <h3 className="title-md text-foreground mb-4">
        {t("actionDetail.scientificReferences")}
      </h3>
...
export default function ActionDetail() {
  ...
  const [servingsNoteContent, setServingsNoteContent] = useState<
    string | undefined
  >();
  ...
  const benefits =
    benefitsList?.items?.map((item: any) => item.description) || [];
  ...
  const whatToEatList = lists.find(
    (l: any) =>
      l.title === "What to eat" ||
  ...
  const thingsToWatchList = lists.find(
    (l: any) => l.title === "Things to watch for",
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-6 py-6 space-y-6">
        ...
        {benefits.length > 0 &&
          (whatToEat.length > 0 || goodToKnowItems.length > 0) && <Separator />}
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <h3 className="title-md text-foreground">
                {goodToKnowList?.title || t("actionDetail.goodToKnow")}
              </h3>
              {goodToKnowInfo && (
                <Button
                  variant="ghost"
          ...
          <>
            <div className="flex gap-3 pt-2">
              ...
              <div className="flex-1">
                <h4 className="title-sm text-foreground mb-1">
                  {whatToEatTip.title}
                </h4>
                <p className="body-sm text-muted-foreground">
                  {whatToEatTip.description}
                </p>
              </div>
            </div>
          </>
        ...
        {thingsToWatch.length > 0 && (
          <>
            ...
            <div>
              <h3 className="title-md text-foreground mb-4">
                {t("actionDetail.thingsToWatchFor")}
              </h3>
              <div className="space-y-2">
